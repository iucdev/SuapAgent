    ...
    public static class Float
    {
        ...
        public static float FromByteArray(byte[] bytes)
        {
            ...
            else
            {
                ...
                int exd = txt.Substring(1, 8).BinStringToInt32();
                string ma = txt.Substring(9, 23);
                float mantisse = 1;
                float faktor = (float)1.0;

                //das ist die Anzahl der restlichen bit's
                for (int cnt = 0; cnt <= 22; cnt++)
                {
                    faktor = faktor / (float)2.0;
                    //entspricht 2^-y
                ...
                return (float)(Math.Pow((-1), vz) * Math.Pow(2, (exd - 127)) * mantisse);
            }
        }
...
//
        ...
        public static byte[] ToByteArray(float value)
        {
            ...
            if (wert != 0f)
            {
                ...
                int exponent = (int) Math.Floor(Math.Log(wert)/Math.Log(2.0));
                wert = (float) (wert/(Math.Pow(2, exponent)) - 1);
                ...
                for (int cnt = 1; cnt <= 23; cnt++)
                {
                    ...
                    {
                        wert = (float) (wert - System.Math.Pow(2, -cnt));
                        binString += "1";
                    }
                    else
                        binString += "0";
...
//                values[cnt] =
//                    FromByteArray(new byte[] {bytes[counter++], bytes[counter++], bytes[counter++], bytes[counter++]});
//
//            return values;
//        }
//
